    ...
    if($user['rank'] >= 1){
      if(isset($_POST['article_title'], $_POST['article_section'], $_POST['article_id'])) {
        if(!empty($_POST['article_title']) AND !empty($_POST['article_section']) AND !empty($_POST['article_id'])) {
          ...
          $article_insert = $db->prepare("UPDATE articles SET title = ?, descr = ?, img = ?, section = ?, pin = ? WHERE id = ?");
          $article_insert->execute(array($article_title, $article_descr, $article_img, $article_section, $article_pin, $article_id));
...
case 'configuration':
    if(isset($_SESSION['id'])) {
        if($user['rank'] == 2){
          if(isset($_POST['config_wsname'], $_POST['config_wsdescr'], $_POST['config_wslink'], $_POST['config_wslang'], $_POST['config_wstheme'], $_POST['config_register'])) {
            if(!empty($_POST['config_wsname']) AND !empty($_POST['config_wsdescr']) AND !empty($_POST['config_wslink']) AND !empty($_POST['config_wslang']) AND !empty($_POST['config_wstheme']) AND !empty($_POST['config_register'])) {
              ...
              $config_insert = $db->prepare("UPDATE config SET wsname = ?, wsdescr = ?, wslink = ?, wslang = ?, wstheme = ?, register = ? WHERE id = ?");
              $config_insert->execute(array($config_wsname, $config_wsdescr, $config_wslink, $config_wslang, $config_wstheme, $config_register, 1));
