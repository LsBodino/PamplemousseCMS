<?php
$l_accessdenied = $lg['l_accessdenied'];
$smarty->assign("l_accessdenied", $l_accessdenied);

$l_accountcreated = $lg['l_accountcreated'];
$smarty->assign("l_accountcreated", $l_accountcreated);

$l_admin = $lg['l_admin'];
$smarty->assign("l_admin", $l_admin);

$l_allarticles = $lg['l_allarticles'];
$smarty->assign("l_allarticles", $l_allarticles);

$l_and = $lg['l_and'];
$smarty->assign("l_and", $l_and);

$l_announce = $lg['l_announce'];
$smarty->assign("l_announce", $l_announce);

$l_articlepin = $lg['l_articlepin'];
$smarty->assign("l_articlepin", $l_articlepin);

$l_articlepinned = $lg['l_articlepinned'];
$smarty->assign("l_articlepinned", $l_articlepinned);

$l_articleposted = $lg['l_articleposted'];
$smarty->assign("l_articleposted", $l_articleposted);

$l_articleupdated = $lg['l_articleupdated'];
$smarty->assign("l_articleupdated", $l_articleupdated);

$l_articles = $lg['l_articles'];
$smarty->assign("l_articles", $l_articles);

$l_backwebsite = $lg['l_backwebsite'];
$smarty->assign("l_backwebsite", $l_backwebsite);

$l_badrequest = $lg['l_badrequest'];
$smarty->assign("l_badrequest", $l_badrequest);

$l_ban = $lg['l_ban'];
$smarty->assign("l_ban", $l_ban);

$l_banishoneself = $lg['l_banishoneself'];
$smarty->assign("l_banishoneself", $l_banishoneself);

$l_banned = $lg['l_banned'];
$smarty->assign("l_banned", $l_banned);

$l_by = $lg['l_by'];
$smarty->assign("l_by", $l_by);

$l_cannotchangesuperadmin = $lg['l_cannotchangesuperadmin'];
$smarty->assign("l_cannotchangesuperadmin", $l_cannotchangesuperadmin);

$l_categories = $lg['l_categories'];
$smarty->assign("l_categories", $l_categories);

$l_category = $lg['l_category'];
$smarty->assign("l_category", $l_category);

$l_categoryposted = $lg['l_categoryposted'];
$smarty->assign("l_categoryposted", $l_categoryposted);

$l_categorysametag = $lg['l_categorysametag'];
$smarty->assign("l_categorysametag", $l_categorysametag);

$l_categoryupdated = $lg['l_categoryupdated'];
$smarty->assign("l_categoryupdated", $l_categoryupdated);

$l_config = $lg['l_config'];
$smarty->assign("l_config", $l_config);

$l_create = $lg['l_create'];
$smarty->assign("l_create", $l_create);

$l_createarticle = $lg['l_createarticle'];
$smarty->assign("l_createarticle", $l_createarticle);

$l_createcategory = $lg['l_createcategory'];
$smarty->assign("l_createcategory", $l_createcategory);

$l_createpage = $lg['l_createpage'];
$smarty->assign("l_createpage", $l_createpage);

$l_defaultcategory = $lg['l_defaultcategory'];
$smarty->assign("l_defaultcategory", $l_defaultcategory);

$l_delete = $lg['l_delete'];
$smarty->assign("l_delete", $l_delete);

$l_descr = $lg['l_descr'];
$smarty->assign("l_descr", $l_descr);

$l_descrmax = $lg['l_descrmax'];
$smarty->assign("l_descrmax", $l_descrmax);

$l_dontaccount = $lg['l_dontaccount'];
$smarty->assign("l_dontaccount", $l_dontaccount);

$l_edit = $lg['l_edit'];
$smarty->assign("l_edit", $l_edit);

$l_editarticle = $lg['l_editarticle'];
$smarty->assign("l_editarticle", $l_editarticle);

$l_editcategory = $lg['l_editcategory'];
$smarty->assign("l_editcategory", $l_editcategory);

$l_editor = $lg['l_editor'];
$smarty->assign("l_editor", $l_editor);

$l_editpage = $lg['l_editpage'];
$smarty->assign("l_editpage", $l_editpage);

$l_editrank = $lg['l_editrank'];
$smarty->assign("l_editrank", $l_editrank);

$l_edituser = $lg['l_edituser'];
$smarty->assign("l_edituser", $l_edituser);

$l_email = $lg['l_email'];
$smarty->assign("l_email", $l_email);

$l_emailerror = $lg['l_emailerror'];
$smarty->assign("l_emailerror", $l_emailerror);

$l_emailused = $lg['l_emailused'];
$smarty->assign("l_emailused", $l_emailused);

$l_entitytoolarge = $lg['l_entitytoolarge'];
$smarty->assign("l_entitytoolarge", $l_entitytoolarge);

$l_error = $lg['l_error'];
$smarty->assign("l_error", $l_error);

$l_errorserver = $lg['l_errorserver'];
$smarty->assign("l_errorserver", $l_errorserver);

$l_general = $lg['l_general'];
$smarty->assign("l_general", $l_general);

$l_haveaccount = $lg['l_haveaccount'];
$smarty->assign("l_haveaccount", $l_haveaccount);

$l_homepage = $lg['l_homepage'];
$smarty->assign("l_homepage", $l_homepage);

$l_httpnotsupported = $lg['l_httpnotsupported'];
$smarty->assign("l_httpnotsupported", $l_httpnotsupported);

$l_id = $lg['l_id'];
$smarty->assign("l_id", $l_id);

$l_image = $lg['l_image'];
$smarty->assign("l_image", $l_image);

$l_impamplemousse = $lg['l_impamplemousse'];
$smarty->assign("l_impamplemousse", $l_impamplemousse);

$l_in = $lg['l_in'];
$smarty->assign("l_in", $l_in);

$l_lang = $lg['l_lang'];
$smarty->assign("l_lang", $l_lang);

$l_lastlogin = $lg['l_lastlogin'];
$smarty->assign("l_lastlogin", $l_lastlogin);

$l_latestarticlesof = $lg['l_latestarticlesof'];
$smarty->assign("l_latestarticlesof", $l_latestarticlesof);

$l_letsgo = $lg['l_letsgo'];
$smarty->assign("l_letsgo", $l_letsgo);

$l_link = $lg['l_link'];
$smarty->assign("l_link", $l_link);

$l_list = $lg['l_list'];
$smarty->assign("l_list", $l_list);

$l_listarticles = $lg['l_listarticles'];
$smarty->assign("l_listarticles", $l_listarticles);

$l_listcategories = $lg['l_listcategories'];
$smarty->assign("l_listcategories", $l_listcategories);

$l_listpages = $lg['l_listpages'];
$smarty->assign("l_listpages", $l_listpages);

$l_listranks = $lg['l_listranks'];
$smarty->assign("l_listranks", $l_listranks);

$l_listusers = $lg['l_listusers'];
$smarty->assign("l_listusers", $l_listusers);

$l_login = $lg['l_login'];
$smarty->assign("l_login", $l_login);

$l_logout = $lg['l_logout'];
$smarty->assign("l_logout", $l_logout);

$l_map = $lg['l_map'];
$smarty->assign("l_map", $l_map);

$l_member = $lg['l_member'];
$smarty->assign("l_member", $l_member);

$l_methodnotallowed = $lg['l_methodnotallowed'];
$smarty->assign("l_methodnotallowed", $l_methodnotallowed);

$l_mostrecentarticles = $lg['l_mostrecentarticles'];
$smarty->assign("l_mostrecentarticles", $l_mostrecentarticles);

$l_mostrecentpages = $lg['l_mostrecentpages'];
$smarty->assign("l_mostrecentpages", $l_mostrecentpages);

$l_mostrecentusers = $lg['l_mostrecentusers'];
$smarty->assign("l_mostrecentusers", $l_mostrecentusers);

$l_myspace = $lg['l_myspace'];
$smarty->assign("l_myspace", $l_myspace);

$l_name = $lg['l_name'];
$smarty->assign("l_name", $l_name);

$l_namemax = $lg['l_namemax'];
$smarty->assign("l_namemax", $l_namemax);

$l_namemax2 = $lg['l_namemax2'];
$smarty->assign("l_namemax2", $l_namemax2);

$l_namemin = $lg['l_namemin'];
$smarty->assign("l_namemin", $l_namemin);

$l_newemail = $lg['l_newemail'];
$smarty->assign("l_newemail", $l_newemail);

$l_newprofilepic = $lg['l_newprofilepic'];
$smarty->assign("l_newprofilepic", $l_newprofilepic);

$l_newpw = $lg['l_newpw'];
$smarty->assign("l_newpw", $l_newpw);

$l_newpwc = $lg['l_newpwc'];
$smarty->assign("l_newpwc", $l_newpwc);

$l_newusername = $lg['l_newusername'];
$smarty->assign("l_newusername", $l_newusername);

$l_no = $lg['l_no'];
$smarty->assign("l_no", $l_no);

$l_noarticle = $lg['l_noarticle'];
$smarty->assign("l_noarticle", $l_noarticle);

$l_nocategory = $lg['l_nocategory'];
$smarty->assign("l_nocategory", $l_nocategory);

$l_nopage = $lg['l_nopage'];
$smarty->assign("l_nopage", $l_nopage);

$l_notextended = $lg['l_notextended'];
$smarty->assign("l_notextended", $l_notextended);

$l_notfound = $lg['l_notfound'];
$smarty->assign("l_notfound", $l_notfound);

$l_notimplemented = $lg['l_notimplemented'];
$smarty->assign("l_notimplemented", $l_notimplemented);

$l_notregistered = $lg['l_notregistered'];
$smarty->assign("l_notregistered", $l_notregistered);

$l_notregisteredsection1 = $lg['l_notregisteredsection1'];
$smarty->assign("l_notregisteredsection1", $l_notregisteredsection1);

$l_notregisteredsection2 = $lg['l_notregisteredsection2'];
$smarty->assign("l_notregisteredsection2", $l_notregisteredsection2);

$l_ok = $lg['l_ok'];
$smarty->assign("l_ok", $l_ok);

$l_pagemenu = $lg['l_pagemenu'];
$smarty->assign("l_pagemenu", $l_pagemenu);

$l_pageposted = $lg['l_pageposted'];
$smarty->assign("l_pageposted", $l_pageposted);

$l_pageupdated = $lg['l_pageupdated'];
$smarty->assign("l_pageupdated", $l_pageupdated);

$l_pages = $lg['l_pages'];
$smarty->assign("l_pages", $l_pages);

$l_pamplemoussecms = $lg['l_pamplemoussecms'];
$smarty->assign("l_pamplemoussecms", $l_pamplemoussecms);

$l_panel = $lg['l_panel'];
$smarty->assign("l_panel", $l_panel);

$l_paneltheme = $lg['l_paneltheme'];
$smarty->assign("l_paneltheme", $l_paneltheme);

$l_panelthemenotfound = $lg['l_panelthemenotfound'];
$smarty->assign("l_panelthemenotfound", $l_panelthemenotfound);

$l_permissions = $lg['l_permissions'];
$smarty->assign("l_permissions", $l_permissions);

$l_powered = $lg['l_powered'];
$smarty->assign("l_powered", $l_powered);

$l_publish = $lg['l_publish'];
$smarty->assign("l_publish", $l_publish);

$l_published = $lg['l_published'];
$smarty->assign("l_published", $l_published);

$l_pw = $lg['l_pw'];
$smarty->assign("l_pw", $l_pw);

$l_pwc = $lg['l_pwc'];
$smarty->assign("l_pwc", $l_pwc);

$l_pwerror = $lg['l_pwerror'];
$smarty->assign("l_pwerror", $l_pwerror);

$l_pwmin = $lg['l_pwmin'];
$smarty->assign("l_pwmin", $l_pwmin);

$l_rank = $lg['l_rank'];
$smarty->assign("l_rank", $l_rank);

$l_ranks = $lg['l_ranks'];
$smarty->assign("l_ranks", $l_ranks);

$l_rankupdated = $lg['l_rankupdated'];
$smarty->assign("l_rankupdated", $l_rankupdated);

$l_read = $lg['l_read'];
$smarty->assign("l_read", $l_read);

$l_recover = $lg['l_recover'];
$smarty->assign("l_recover", $l_recover);

$l_register = $lg['l_register'];
$smarty->assign("l_register", $l_register);

$l_registrationdate = $lg['l_registrationdate'];
$smarty->assign("l_registrationdate", $l_registrationdate);

$l_registrationsclosed = $lg['l_registrationsclosed'];
$smarty->assign("l_registrationsclosed", $l_registrationsclosed);

$l_registrationwebsite = $lg['l_registrationwebsite'];
$smarty->assign("l_registrationwebsite", $l_registrationwebsite);

$l_rememberme = $lg['l_rememberme'];
$smarty->assign("l_rememberme", $l_rememberme);

$l_requesttimeout = $lg['l_requesttimeout'];
$smarty->assign("l_requesttimeout", $l_requesttimeout);

$l_section = $lg['l_section'];
$smarty->assign("l_section", $l_section);

$l_settings = $lg['l_settings'];
$smarty->assign("l_settings", $l_settings);

$l_settingsupdated = $lg['l_settingsupdated'];
$smarty->assign("l_settingsupdated", $l_settingsupdated);

$l_smarty = $lg['l_smarty'];
$smarty->assign("l_smarty", $l_smarty);

$l_space = $lg['l_space'];
$smarty->assign("l_space", $l_space);

$l_spaceof = $lg['l_spaceof'];
$smarty->assign("l_spaceof", $l_spaceof);

$l_success = $lg['l_success'];
$smarty->assign("l_success", $l_success);

$l_tag = $lg['l_tag'];
$smarty->assign("l_tag", $l_tag);

$l_theme = $lg['l_theme'];
$smarty->assign("l_theme", $l_theme);

$l_themenotfound = $lg['l_themenotfound'];
$smarty->assign("l_themenotfound", $l_themenotfound);

$l_toomanyrequest = $lg['l_toomanyrequest'];
$smarty->assign("l_toomanyrequest", $l_toomanyrequest);

$l_trash = $lg['l_trash'];
$smarty->assign("l_trash", $l_trash);

$l_unban = $lg['l_unban'];
$smarty->assign("l_unban", $l_unban);

$l_unsupportedmediatype = $lg['l_unsupportedmediatype'];
$smarty->assign("l_unsupportedmediatype", $l_unsupportedmediatype);

$l_username = $lg['l_username'];
$smarty->assign("l_username", $l_username);

$l_usernamemax = $lg['l_usernamemax'];
$smarty->assign("l_usernamemax", $l_usernamemax);

$l_usernamemin = $lg['l_usernamemin'];
$smarty->assign("l_usernamemin", $l_usernamemin);

$l_usernameunauthorized = $lg['l_usernameunauthorized'];
$smarty->assign("l_usernameunauthorized", $l_usernameunauthorized); 

$l_usernameused = $lg['l_usernameused'];
$smarty->assign("l_usernameused", $l_usernameused);

$l_users = $lg['l_users'];
$smarty->assign("l_users", $l_users);

$l_version = $lg['l_version'];
$smarty->assign("l_version", $l_version);

$l_website = $lg['l_website'];
$smarty->assign("l_website", $l_website);

$l_welcometo = $lg['l_welcometo'];
$smarty->assign("l_welcometo", $l_welcometo);

$l_wrongemailpw = $lg['l_wrongemailpw'];
$smarty->assign("l_wrongemailpw", $l_wrongemailpw);

$l_yes = $lg['l_yes'];
$smarty->assign("l_yes", $l_yes);

$l_youarebanned = $lg['l_youarebanned'];
$smarty->assign("l_youarebanned", $l_youarebanned);
?>